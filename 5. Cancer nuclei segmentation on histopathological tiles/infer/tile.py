...
def GET_file_list(Path):
    ...
    for root, dirs, files in os.walk(Path):
        if len(dirs)==0 and len(files) !=0:
            name_list +=glob.glob(os.path.join(root,'*.png'))
    return name_list
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        file_path_list = GET_file_list(self.input_dir)
        file_path_list.sort()  # ensure same order
        # target_list = copy.deepcopy(file_path_list)
        ALL_NUM=len(file_path_list)
        assert len(file_path_list) > 0, 'Not Detected Any Files From Path'
        ## if continue process be according to previous process
        if not os.path.isdir(self.output_dir + '/mat/'):
            Continue_Pre=False
        else:
            Continue_Pre = True
            mat_list = os.listdir(self.output_dir + '/mat/')
            overlay_list = os.listdir(self.output_dir + '/overlay/')



        # rm_n_mkdir(self.output_dir + '/json/')
        mkdir(self.output_dir + '/mat/')
        mkdir(self.output_dir + '/overlay/')
        # if self.save_qupath:
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                pbar_all.update()
                ##### if continue process according to previous result
                if Continue_Pre:
                    IMG_NAME = os.path.basename(file_path)
                    MAT_NAME = IMG_NAME.replace('.png','.mat')
                    # if (IMG_NAME in overlay_list) and (MAT_NAME in mat_list):
                    if MAT_NAME in mat_list:
                        continue
                img = cv2.imread(file_path)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                src_shape = img.shape
                nr_pixels = src_shape[0] * src_shape[1]
                # Make the number of pixels  within the appropriate range
                if nr_pixels < MIN_NR_PIXELS or nr_pixels > MAX_NR_PIXELS:
                    continue
            ...
            # pbar = tqdm.tqdm(
            #     desc=f"Process Patches:{BATCH_NUM} ",
            #     leave=True,
