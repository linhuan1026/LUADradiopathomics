...
if __name__ == '__main__':
     ...
     # 需要设定的参数：
    EPI_CSV_PATH = '/media/gzzstation/14TB/PyRadiomics_Results/example3/epi_csv/XY2-2_10x_epi_feat_csv/'
    STROMA_CSV_PATH = '/media/gzzstation/14TB/PyRadiomics_Results/example4/stroma_csv/XY2-2_10x_Stroma_feat_csv/'
    center='XY2-2'
    SAVE_CSV_PATH='./'+center+'_epi+stroma.csv'
    ...
    for N in NUM:
        ...
        wsi_path = sorted(os.listdir(EPI_CSV_PATH))

        for csv_path in wsi_path:

            # 当前sample的路径：
            cur_sample_path = EPI_CSV_PATH+csv_path
            # 开始计算当前sample的统计学特征：
            final_list, final_col_name = sta_charcter_calculation(cur_sample_path, N, center, tissue)
    ...
    for N in NUM:
        wsi_path = sorted(os.listdir(STROMA_CSV_PATH))

        for csv_path in wsi_path:

            # 当前sample的路径：
            cur_sample_path = STROMA_CSV_PATH+csv_path
            # 开始计算当前sample的统计学特征：
    ...
    print("combine final csv successfully !")
