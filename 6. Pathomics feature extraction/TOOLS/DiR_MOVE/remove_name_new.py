...
new_mat_path='/media/gzzstation/14TB/PyRadiomics_Data/SXCH/mat_40x_rename/'
...
for wsi_path in wsi_paths:
    ...
    for img_patch in patch_id:
        ...
        if pid_patch.endswith('_new') is True:
            pid_patch_right=pid_patch[:-4]
        else:
            pid_patch_right=pid_patch
        
        src_path=base_dir+patient_id+'/'+img_patch

        save_dir = new_mat_path+patient_id+'/'
        os.makedirs(save_dir, exist_ok=True)

        tar_path=new_mat_path+patient_id+'/'+pid_patch_right+'.mat'
