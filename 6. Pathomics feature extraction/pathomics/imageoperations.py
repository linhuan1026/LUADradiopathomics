...
def getMask(mask, **kwargs):
    ...
    if label not in labels:
        raise ValueError('Label (%g) not present in mask. Choose from %s' %
...
def getBinEdges(parameterValues, **kwargs):
    ...
    **Fixed bin Count:**
    ...
    if binCount is not None:
        ...
        binEdges[
            -1] += 1  # Ensures that the maximum value is included in the topmost bin when using numpy.digitize
    else:
        minimum = np.min(parameterValues)
        maximum = np.max(parameterValues)
        ...
        if len(binEdges) == 1:  # Flat region, ensure that there is 1 bin
            binEdges = [
                binEdges[0] - .5, binEdges[0] + .5
            ]  # Simulates binEdges returned by numpy.histogram if bins = 1

        logger.debug('Calculated %d bins for bin width %g with edges: %s)',
                     len(binEdges) - 1, binWidth, binEdges)
...
def binImage(parameterMatrix, parameterMatrixCoordinates=None, **kwargs):
    ...
    discretizedParameterMatrix = numpy.zeros(parameterMatrix.shape,
                                             dtype='int')
    if parameterMatrixCoordinates is None:
        ...
        binEdges = getBinEdges(parameterMatrix[parameterMatrixCoordinates],
                               **kwargs)
        discretizedParameterMatrix[
            parameterMatrixCoordinates] = numpy.digitize(
...
def checkMask(imageNode, maskNode, **kwargs):
    ...
    try:
        ...
        if not kwargs.get('correctMask', False):
            if "Both images for LabelStatisticsImageFilter don't match type or dimension!" in e.args[
                    0]:
        ...
        correctedMask = _correctMask(
            imageNode, maskNode,
            **kwargs)  # Raises Value error if ROI outside image physical space

        # Resampling successful, try to calculate boundingbox
        try:
            lsif.Execute(imageNode, correctedMask)
        except RuntimeError:
            logger.debug('Bounding box calculation with resampled mask failed',
                         exc_info=True)
            raise ValueError(
    ...
    logger.debug('Checking minimum number of dimensions requirements (%d)',
                 minDims)
    ndims = numpy.sum((boundingBox[1::2] - boundingBox[0::2] + 1) >
                      1)  # UBound - LBound + 1 = Size
    if ndims == 0:
        raise ValueError(
    ...
    if minSize is not None:
        logger.debug('Checking minimum size requirements (minimum size: %d)',
                     minSize)
...
def _correctMask(imageNode, maskNode, **kwargs):
    ...
    _checkROI(imageNode, maskNode,
              **kwargs)  # Raises a value error if ROI is invalid
...
def _checkROI(imageNode, maskNode, **kwargs):
    ...
    ROIBounds = (
        maskNode.TransformContinuousIndexToPhysicalPoint(bb[:Nd] -
                                                         .5),  # Origin
        maskNode.TransformContinuousIndexToPhysicalPoint(bb[:Nd] + bb[Nd:] -
                                                         0.5))  # UBound
    # Step 2: Translate the ROI physical bounds to the image coordinate space
    ROIBounds = (
        imageNode.TransformPhysicalPointToContinuousIndex(
            ROIBounds[0]),  # Origin
...
def cropToTumorMask(imageNode, maskNode, boundingBox, **kwargs):
    ...
    # Crop Image
    logger.debug('Cropping to size %s',
                 (boundingBox[1::2] - boundingBox[0::2]) + 1)
...
def resampleImage(imageNode, maskNode, **kwargs):
    ...
    resampledPixelSpacing = numpy.where(resampledPixelSpacing == 0,
                                        maskSpacing, resampledPixelSpacing)
    ...
    resampledPixelSpacing = numpy.where(bb[Nd_mask:] != 1,
                                        resampledPixelSpacing, maskSpacing)
    ...
    bbNewLBound = numpy.floor((bb[:Nd_mask] - 0.5) * spacingRatio -
                              padDistance)
    bbNewUBound = numpy.ceil((bb[:Nd_mask] + bb[Nd_mask:] - 0.5) *
                             spacingRatio + padDistance)
    ...
    newOriginIndex = numpy.array(.5 * (resampledPixelSpacing - maskSpacing) /
                                 maskSpacing)
    newCroppedOriginIndex = newOriginIndex + bbOriginalLBound
    newOrigin = maskNode.TransformContinuousIndexToPhysicalPoint(
        newCroppedOriginIndex)
    ...
    try:
        ...
        logger.warning('interpolator "%s" not recognized, using sitkBSpline',
                       interpolator)
...
def resegmentMask(imageNode, maskNode, **kwargs):
    ...
    if len(resegmentRange) == 0 or len(resegmentRange) > 2:
        raise ValueError('Length %i is not allowed for resegmentRange' %
                         len(resegmentRange))

    logger.debug('Resegmenting mask (range %s, mode %s)', resegmentRange,
                 resegmentMode)
    ...
    if resegmentMode == 'absolute':
        ...
        logger.debug('Resegmenting in sigma mode, mean %g, std %g', mean_gl,
                     sd_gl)
...
def getLoGImage(inputImage, inputMask, **kwargs):
    ...
    for sigma in sigmaValues:
        ...
        if sigma > 0.0:
            ...
            logger.warning('applyLoG: sigma must be greater than 0.0: %g',
                           sigma)
...
def getLBP2DImage(inputImage, inputMask, **kwargs):
    ...
    if Nd == 3:
        ...
        im_arr = local_binary_pattern(im_arr,
                                      P=lbp_samples,
                                      R=lbp_radius,
                                      method=lbp_method)
    else:
...
def getLBP3DImage(inputImage, inputMask, **kwargs):
    ...
    if Nd != 3:
        logger.warning(
            'LBP 3D only available for 3 dimensional images, found %i dimensions',
            Nd)
        return
    ...
    coords_icosahedron = numpy.array(
        icosphere(lbp_icosphereSubdivision,
                  lbp_icosphereRadius).vertices)  # shape(Nv, 3)
    ...
    coords = numpy.array(ROI_coords).T[
        None, :, :] + coords_icosahedron[:, None, :]  # shape(Nv, Np, 3)
    f = map_coordinates(
    ...
    f = numpy.greater_equal(f, f_centroids[:,
                                           None]).astype(int)  # Shape(Np, Nv)
    ...
    # Integrate over m
    f = numpy.multiply(c[:, None, n_ix == 0],
                       Y[None, :, n_ix == 0])  # Shape (Np, Nv, 1)
    for n in range(1, lbp_levels):
        f = numpy.concatenate(
            (f,
             numpy.sum(numpy.multiply(c[:, None, n_ix == n], Y[None, :,
                                                               n_ix == n]),
                       axis=2,
                       keepdims=True)),
            axis=2)
