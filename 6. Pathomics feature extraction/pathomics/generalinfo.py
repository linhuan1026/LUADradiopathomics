...
class GeneralInfo:
    ...
    def addStaticElements(self):
        ...
        - Version: current version of PyPathomics
        - NumpyVersion: version of numpy used
        - SimpleITKVersion: version SimpleITK used
        - PyWaveletVersion: version of PyWavelet used
        - PythonVersion: version of the python interpreter running PyPathomics
        """

        self.generalInfo[self.generalInfo_prefix +
                         'Versions_PyPathomics'] = pathomics.__version__
        self.generalInfo[self.generalInfo_prefix +
                         'Versions_Numpy'] = numpy.__version__
        self.generalInfo[
            self.generalInfo_prefix +
            'Versions_SimpleITK'] = sitk.Version().VersionString()
        self.generalInfo[self.generalInfo_prefix +
                         'Versions_PyWavelet'] = pywt.__version__
        self.generalInfo[self.generalInfo_prefix +
                         'Versions_Python'] = '%i.%i.%i' % sys.version_info[:3]
    ...
    def addImageElements(self, image, prefix='original'):
        ...
        if prefix == 'original':
            self.generalInfo[self.generalInfo_prefix +
                             'Image-original_Hash'] = sitk.Hash(image)
            self.generalInfo[
                self.generalInfo_prefix +
                'Image-original_Dimensionality'] = '%iD' % image.GetDimension(
                )

        self.generalInfo[self.generalInfo_prefix + 'Image-' + prefix +
                         '_Spacing'] = image.GetSpacing()
        self.generalInfo[self.generalInfo_prefix + 'Image-' + prefix +
                         '_Size'] = image.GetSize()
        im_arr = sitk.GetArrayFromImage(image).astype('float')
        self.generalInfo[self.generalInfo_prefix + 'Image-' + prefix +
                         '_Mean'] = numpy.mean(im_arr)
        self.generalInfo[self.generalInfo_prefix + 'Image-' + prefix +
                         '_Minimum'] = numpy.min(im_arr)
        self.generalInfo[self.generalInfo_prefix + 'Image-' + prefix +
                         '_Maximum'] = numpy.max(im_arr)
    ...
    def addMaskElements(self, image, mask, label, prefix='original'):
        ...
        if prefix == 'original':
            self.generalInfo[self.generalInfo_prefix +
                             'Mask-original_Hash'] = sitk.Hash(mask)

        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_Spacing'] = mask.GetSpacing()
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_Size'] = mask.GetSize()
        ...
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_BoundingBox'] = lssif.GetBoundingBox(label)
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_VoxelNum'] = lssif.GetNumberOfPixels(label)
        ...
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_VolumeNum'] = ccif.GetObjectCount()

        ma_arr = sitk.GetArrayFromImage(labelMap) == 1
        maskCoordinates = numpy.array(numpy.where(ma_arr))
        center_index = tuple(
            numpy.mean(maskCoordinates,
                       axis=1)[::-1])  # also convert z, y, x to x, y, z order

        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_CenterOfMassIndex'] = center_index

        self.generalInfo[
            self.generalInfo_prefix + 'Mask-' + prefix +
            '_CenterOfMass'] = mask.TransformContinuousIndexToPhysicalPoint(
                center_index)
        ...
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_Mean'] = numpy.mean(targetvoxels)
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_Minimum'] = numpy.min(targetvoxels)
        self.generalInfo[self.generalInfo_prefix + 'Mask-' + prefix +
                         '_Maximum'] = numpy.max(targetvoxels)
    ...
    def addEnabledImageTypes(self, enabledImageTypes):
        ...
        """
        self.generalInfo[self.generalInfo_prefix +
                         'Configuration_EnabledImageTypes'] = enabledImageTypes
