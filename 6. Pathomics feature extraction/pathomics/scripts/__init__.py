...
import pathomics.featureextractor
...
class PyRadiomicsCommandLine:
    ...
    def __init__(self, custom_arguments=None):
        self.logger = logging.getLogger(
            'pathomics.script')  # holds logger for script events
        self.relative_path_start = os.getcwd()
        self.args = self.getParser().parse_args(
            args=custom_arguments)  # Exits with code 2 if parsing fails
    ...
    def getParser(cls):
        parser = argparse.ArgumentParser(
            usage='%(prog)s image|batch [mask] [Options]',
        ...
        inputGroup.add_argument(
            ...
            action='store_true',
            help=
            'If specified, check if input is valid and check if file locations point to exisiting '
            'files')

        outputGroup = parser.add_argument_group(
            title='Output',
            description='Arguments controlling output redirection and '
            'the formatting of calculated results.')
        outputGroup.add_argument('--out',
                                 '-o',
                                 metavar='FILE',
        ...
        outputGroup.add_argument(
            ...
            default=None,
            help=
            'Directory to store output. If specified in segment mode, this writes csv output for '
        ...
        outputGroup.add_argument('--mode',
                                 '-m',
                                 choices=['segment', 'voxel'],
                                 default='segment',
        ...
        loggingGroup.add_argument('--logging-level',
                                  ...
                                  default='WARNING',
                                  help='Set capture level for logging')
        loggingGroup.add_argument('--log-file',
                                  metavar='FILE',
                                  default=None,
        ...
        parser.add_argument('--version',
                            ...
                            version='%(prog)s ' + pathomics.__version__)
        return parser
    ...
    def run(self):
        ...
        return
        try:
            self.logger.info('Starting PyRadiomics (version: %s)',
                             pathomics.__version__)
            ...
            if caseGenerator is not None:
                if self.args.validate:
                    ...
                    self.logger.info(
                        'Finished %s-based extraction successfully...',
                        self.args.mode)
            else:
    ...
    def _processInput(self):
        ...
        if self.args.input.endswith('.csv'):
            ...
            with open(self.args.input, mode='r') as batchFile:
                ...
                for row_idx, row in enumerate(cr, start=2):
                    if row['Image'] is None or row['Mask'] is None:
                        self.logger.warning(
                            'Batch L%d: Missing required Image or Mask, skipping this case...',
                            row_idx)
                        continue
                    imPath = row['Image']
                    maPath = row['Mask']
                    if not os.path.isabs(imPath):
                        imPath = os.path.abspath(
                            os.path.join(self.relative_path_start, imPath))
                        self.logger.debug(
                            'Considered image filepath to be relative to input CSV. Absolute path: %s',
                            imPath)
                    if not os.path.isabs(maPath):
                        maPath = os.path.abspath(
                            os.path.join(self.relative_path_start, maPath))
                        self.logger.debug(
                            'Considered mask filepath to be relative to input CSV. Absolute path: %s',
                            maPath)
                    cases.append(row)
            ...
            caseGenerator = [(1, {
                'Image': self.args.input,
                'Mask': self.args.mask
            })]
        else:
            self.logger.error(
                'Input is not recognized as batch, no mask specified, cannot compute result!'
            )
            return None
        ...
        for case_idx, case in case_generator:
            if case_idx == 1 and self.args.param is not None:
                if not os.path.isfile(self.args.param):
                    ...
                    schemaFile, schemaFuncs = pathomics.getParameterValidationFiles(
                    )

                    c = pykwalify.core.Core(source_file=self.args.param,
                    ...
                    try:
                        ...
                        self.logger.error('Parameter validation failed!',
                                          exc_info=True)
                        self.logger.debug("Validating case (%i/%i): %s",
                                          case_idx, self.case_count, case)

            case_error = False
            if not os.path.isfile(case['Image']):
                case_error = True
                self.logger.error(
                    'Image path for case (%i/%i) does not exist!', case_idx,
                    self.case_count)
            if not os.path.isfile(case['Mask']):
                case_error = True
                self.logger.error('Mask path for case (%i/%i) does not exist!',
                                  case_idx, self.case_count)

            if case_error:
                errored_cases += 1

        self.logger.info('Validation complete, errors found in %i case(s)',
                         errored_cases)

    def _processCases(self, case_generator):
        setting_overrides = self._parseOverrides()

        extractor = pathomics.featureextractor.RadiomicsFeatureExtractor(
            self.args.param, **setting_overrides)

        if self.args.out_dir is not None and not os.path.isdir(
                self.args.out_dir):
            os.makedirs(self.args.out_dir)

        if self.num_workers > 1:  # multiple cases, parallel processing enabled
            self.logger.info(
                'Input valid, starting parallel extraction from %d cases with %d workers...',
                self.case_count, self.num_workers)
            pool = Pool(self.num_workers)
            try:
                task = pool.map_async(partial(
                    self.parallel_func,
                    extractor=extractor,
            ...
            self.logger.info(
                'Input valid, starting sequential extraction from %d case(s)...',
                self.case_count)
            results = []
            for case in case_generator:
                results.append(
                    self.serial_func(*case,
                                     extractor=extractor,
                                     out_dir=self.args.out_dir,
                                     unix_path=self.args.unix_path))
        else:
    ...
    def _processOutput(self, results):
        ...
        # preserved. Additional headers are by definition generated by pypathomics, and therefore can be appended at
        # the end.
        additional_headers = set()
        for case in results[1:]:
            additional_headers.update(set(case.keys()))
            additional_headers -= set(results[0].keys(
            ))  # Subtract all headers found in the first case
        ...
        if self.args.format_path == 'absolute':
            ...
            pathFormatter = partial(os.path.relpath,
                                    start=self.relative_path_start)
        elif self.args.format_path == 'basename':
            pathFormatter = os.path.basename
        else:
            self.logger.warning(
                'Unrecognized format for paths (%s), reverting to default ("absolute")',
        ...
        for case_idx, case in enumerate(results, start=1):
            ...
            if self.args.format == 'csv':
                writer = csv.DictWriter(self.args.out,
                                        headers,
                                        lineterminator='\n',
                                        extrasaction='ignore')
                if case_idx == 1:
    ...
    def _parseOverrides(self):
        ...
        with open(schemaFile) as schema:
            settingsSchema = yaml.safe_load(
                schema)['mapping']['setting']['mapping']
        ...
        def parse_value(value, value_type):
            if value_type == 'str':
                ...
                raise ValueError('Cannot understand value_type "%s"' %
                                 value_type)

        for setting in self.args.setting:  # setting = "setting_key:setting_value"
            if ':' not in setting:
                self.logger.warning(
                    'Incorrect format for override setting "%s", missing ":"',
                    setting)
                continue
            ...
            if setting_key not in settingsSchema:
                self.logger.warning(
                    'Did not recognize override "%s", skipping...',
                    setting_key)
                continue
            ...
            try:
                ...
                if 'seq' in setting_def:
                    # Multivalued setting
                    if len(setting_def['seq']
                           ) > 0 and 'type' in setting_def['seq'][0]:
                        setting_type = setting_def['seq'][0]['type']

                    setting_overrides[setting_key] = [
                        parse_value(val, setting_type)
                        for val in setting_value.split(',')
                    ]
                    self.logger.debug(
                        'Parsed "%s" as list (element type "%s"); value: %s',
                        setting_key, setting_type,
                        setting_overrides[setting_key])
                else:
                    if 'type' in setting_def:
                        setting_type = setting_def['type']
                    setting_overrides[setting_key] = parse_value(
                        setting_value, setting_type)
                    self.logger.debug('Parsed "%s" as type "%s"; value: %s',
                                      setting_key, setting_type,
                                      setting_overrides[setting_key])
                ...
                self.logger.warning(
                    'Could not parse value "%s" for setting "%s", skipping...',
    ...
    def _configureLogging(self):
        ...
        verboseLevel = (
            6 - self.args.verbosity) * 10  # convert to python logging level
        ...
        if self.args.log_file is not None:
            ...
            if self.args.jobs > 1 and py_version >= (3, 2):
                ...
                file_handler = logging.FileHandler(filename=self.args.log_file,
                                                   mode='a')
                file_handler.setFormatter(
                    logging.Formatter(fmt=logging_config['formatters']
                                      ['default'].get('format'),
                                      datefmt=logging_config['formatters']
                                      ['default'].get('datefmt')))
            ...
            logging_config['loggers']['pathomics']['handlers'].append(
                'logfile')
...
def parse_args():
    try:
        ...
        logging.getLogger().error("Error executing PyRadiomics command line!",
                                  exc_info=True)
